...
struct Delaunay { // 0-base
  ...
  void divide(int l, int r) {
    ...
    auto gao = [&](int t) {
      pii pt[2] = {p[nw[0]], p[nw[1]]};
      for (auto it : head[nw[t]]) {
        int v = ori(pt[1], pt[0], p[it.id]);
        if (v > 0 or (v == 0 and abs2(pt[t ^ 1] - p[it.id]) < abs2(pt[1] - pt[0])))
          return nw[t] = it.id, true;
      }
      return false;
    };
    while (gao(0) or gao(1));
    ...
    while (true) {
      ...
      for (int t = 0; t < 2; ++t)
          for (auto it : head[nw[t]])
              if (ori(pt[0], pt[1], p[it.id]) > 0 and (ch == -1 or in_cc({pt[0], pt[1], p[ch]}, p[it.id])))
                  ch = it.id, sd = t;
