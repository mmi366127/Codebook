 ...
 */
const int inf = maxc * maxc * 100; // lower_bound unknown
struct Tri;
struct Edge {
  Tri* tri; int side;
  Edge(): tri(0), side(0){}
...
struct Trig { // Triangulation
  Trig() {
    the_root = // Tri should at least contain all points
      new(tris++) Tri(pii(-inf, -inf), pii(inf + inf, -inf), pii(-inf, inf + inf));
  }
  Tri* find(pii p) { return find(the_root, p); }
  void add_point(const pii &p) { add_point(find(the_root, p), p); }
  Tri* the_root;
