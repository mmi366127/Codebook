...
struct WeightGraph { // 1-based
  ...
  vector<int> split_flo(auto &f, int xr) {
    ...
    if (auto pr = it - begin(f); pr % 2 == 1)
      reverse(1 + ALL(f)), it = end(f) - pr;
    auto res = vector(begin(f), it);
    return f.erase(begin(f), it), res;
  }
  void set_match(int u, int v) {
    ...
    set_match(xr, v); f.insert(end(f), ALL(z));
  }
  ...
  void add_blossom(int u, int o, int v) {
    ...
    for (int t : {u, v}) {
      ...
      for (int x = t, y; x != o; x = st[pa[y]])
        f.eb(x), f.eb(y = st[match[x]]), q_push(y);
    }
    flo[b] = f; set_st(b, b);
  ...
  bool matching() {
    ...
    q = queue<int>();
    REP(x, 1, nx) if (st[x] == x and !match[x])
      pa[x] = S[x] = 0, q_push(x);
    ...
    for (;;) {
      ...
      int d = INF;
      REP(b, n + 1, nx) if (st[b] == b and S[b] == 1)
        d = min(d, lab[b] / 2);
      REP(x, 1, nx)
        if (int s = slk[x]; st[x] == x and s and S[x] <= 0)
      ...
      REP(b, n + 1, nx) if (st[b] == b and S[b] >= 0)
        lab[b] += d * (2 - 4 * S[b]);
      REP(x, 1, nx)
        if (int s = slk[x]; st[x] == x and
            s and st[s] != x and E(g[s][x]) == 0)
  ...
  pii solve() {
    ...
    fill(ALL(lab), w_max);
    int n_matches = 0, tot_weight = 0;
    while (matching()) ++n_matches;
    REP(u, 1, n) if (match[u] and match[u] < u)
